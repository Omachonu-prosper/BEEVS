    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
      <li v-for="(crumb, index) in crumbs" :key="index" class="inline-flex items-center">
        <div class="flex items-center">
          ...
          <RouterLink :to="crumb.to" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-blue-600">{{ crumb.text }}</RouterLink>
        </div>
      </li>
    </ol>
  </nav>
